class Store::TillsController < ApplicationController
  before_action :set_store_till, only: [:show, :edit, :update, :destroy]

  # GET /store/tills
  def index
    @store_tills = Store::Till.all
  end
  ...
  # POST /store/tills
  def create
    @store_till = Store::Till.new(store_till_params)
  ...
  private
    ...
    def set_store_till
      @store_till = Store::Till.find(params[:id])
    end
